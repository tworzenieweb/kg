               ...
               <tr>
                 <td class="tb-find"><label>Filtruj</label>
                   <select name="filter" onchange="submitbutton('find');">
                     ...
                     <option value="1" <?=($_POST['filter']=='1')?'selected="selected"':''?>>Szkolenia standardowe</option>
                     <option value="2" <?=($_POST['filter']=='2')?'selected="selected"':''?>>Szkolenia specjalne</option>
                     <option value="3" <?=($_POST['filter']=='3')?'selected="selected"':''?>>Szkolenia ukryte</option>
                   </select>
                 </td>
			...
			<col width="5%" />
          	<thead>
            	<tr>
				   ...
				   <td><a href="javascript:columnsort('title');">Tytu³</a></td>
                   ...
                   <td align="center"><a href="javascript:columnsort('intro_visible');">Zajawka</a></td>
				   <td align="center"><a href="javascript:columnsort('visible');">Widoczne</a></td>
                   <td align="center"><a href="javascript:columnsort('special');">Szko³a</a></td>				   
		  ...
		  <?php 		  	
		    ...
		    if ( is_array( $list_resoults ) ) { 
				foreach( $list_resoults as $object ) { 
						$public_visible   = (($object->title != $object->title_m) or ($object->context != $object->context_m) or ($object->introduction != $object->introduction_m) or ($object->special != $object->special_m) or ($object->visible != $object->visible_m) or ($object->intro_visible != $object->intro_visible_m) or ($object->description_m != $object->description) or ($object->instructors != $object->instructors_m))?1:0;
						$pic_into_visible = ($object->intro_visible)?'<img src="../img/small/tick.png" title="Zajawka widoczna w aktualno¶ciach" width="12" height="12" />':'<img src="../img/small/no-accept.gif" title="Zajawka niewidoczna w aktualno¶ciach" width="12" height="12" />';
						$pic_visible      = ($object->visible)?'<img src="../img/small/tick.png" title="Widoczna w serwisie www" width="12" height="12" />':'<img src="../img/small/no-accept.gif" title="Niewidoczna w serwisie www" width="12" height="12" />';
						$pic_special      = ($object->special)?'<img src="../img/small/tick.png" title="Informacja jest widoczna na lewym marginesie" width="12" height="12" />':'<img src="../img/small/no-accept.gif" title="Informacja niewidoczna na lewym marginesie" width="12" height="12" />';
						$pic_public       = ($public_visible)? '<a href="javascript: void(0);" onclick="return listItemTask(\'cb'.$j.'\',\'publish\')"><img src="../img/small/publish_g.png" title="Kliknij aby opublikowaæ informacje na stronie www" width="12" height="12" /></a>':'&nbsp;';						
						$title_text       = ($public_visible)? '<span class="red-bold">('.$object->prefix_code.') '.$object->title_m.'</span>' : '('.$object->prefix_code.') '.$object->title;
	...
	/* Info */			echo '  <td><img src="../img/small/con_info.png" title="" width="16" height="16" '.createHint(array('<b>Identyfikator:</b> '.$object->id,'<b>Data utworzenia:</b>',$object->create_date,'<b>Data zapisu:</b>',$object->update_date,'<b>Opis:</b>',$object->description,'<b>Prowadz±cy:</b>',$object->instructors),'CAPTION','Informacje dodatkowe').'/></td>';
	...
	/* Zajawka */		echo '  <td align="center"><a href="javascript: void(0);" onclick="return listItemTask(\'cb'.$j.'\',\'change_intro_visible\')">'.$pic_into_visible.'</a></td>';
	...
	/* Specjalna */		echo '  <td align="center"><a href="javascript: void(0);" onclick="return listItemTask(\'cb'.$j.'\',\'change_special\')">'.$pic_special.'</a></td>';
