  		  <h1 class="context-header">Raport wysy³ki  <span>[<?=$page->title?>]</span></h1>
               ...
               <tr>
                 <td class="tb-find"><label>Filtruj</label>
                   <select name="filter" onChange="submitbutton('find');">
                     ...
                     <option value="1" <?=($_POST['filter']=='1')?'selected="selected"':''?>>Niewys³ane z powodu b³êdu</option>
                     <option value="2" <?=($_POST['filter']=='2')?'selected="selected"':''?>>Wys³ane poprawnie</option>
                     <option value="3" <?=($_POST['filter']=='3')?'selected="selected"':''?>>Niewysy³ane</option>
                   </select>
                 </td>
			...
			<col width="50%" />
          	<thead>
            	<tr>
                   ...
                   <td><a href="javascript:columnsort('b.email');">Adres e-mail</a></td>
				   <td align="center"><a href="javascript:columnsort('a.status');">Status</a></td>
                   <td><a href="javascript:columnsort('b.info');">Opis</a></td>				   
                </tr>
            </thead>
			 <tbody>	  
		  <?php 		  	
		    ...
		    if ( is_array( $list_resoults ) ) { 
				foreach( $list_resoults as $object ) { 
						if ($object->status==0) {
							$pic_status = '<img src="../img/small/a_no_send.png" width="12" height="12" title="Wiadomo¶æ nie wysy³ana" />';
						} else {
							$pic_status = ($object->status==1)? '<img src="../img/small/a_send.png" width="12" height="12" title="Wiadomo¶æ wys³ana poprawnie" />':'<img src="../img/small/a_error.png" width="12" height="12" title="B³±d podczas wysy³ki"/>';
						}
						$email = ($object->email =='')?'Adres email zosta³ usuniêty':'<a href="mailto:'.$object->email.'">'.$object->email.'</a>';
									
						echo '<tr class="row'.$i.'">';
						echo '  <td align="right">'.$lp.'</td>';
						echo '  <td><img src="../img/small/con_info.png" title="" width="16" height="16" '.createHint(array('<b>Identyfikator:</b> '.$object->id,'<b>¬ród³o:</b> '.$object->source,'<b>Typ:</b> '.$object->type),'CAPTION','Informacje dodatkowe').'/></td>';
						echo '  <td>'.$email.'</td>';
						echo '  <td align="center">'.$pic_status.'</td>';
						echo '  <td>'.$object->info.'</td>';
						echo '</tr>';
						$i = ($i==1)?0:1;
						$j++; $lp++;
				}
